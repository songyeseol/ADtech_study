...
```디지털 광고를 구매하는데 소프트웨어를 사용하는 것```

RFP, 인력 협상, IO의 절차가 필요한 전통적인 애드 바잉과 대립되는 개념이네요.   
...
**프로그래매틱은 애드 바잉 시스템을 더욱 효율적으로(= 인력업무를 가능한 없애며 낮은금액으로) 처리할 수 있도록 합니다.**    
...
하지만 아직도 캠페인을 최적화시키거나 전략을 계획하는데 인력이 필요하긴합니다.  
프로그래매틱 기술은 아마도 더 적은 광고구매인력을 의미하지 않을까요?  
하지만 그와 동시에 마케터와 판매자가 더욱 정교하고 커스터마이즈된 캠페인을 계획하는데 더 많은 시간을 할애하도록 만들 수도 있겠네요.  
...
**프로그래매틱** 소프트웨어는 광고주들이 미리 특정한 매체 사이트에서 보장된 광고 노출(guaranteed ad impression)을 구매하는 것 또한 포함합니다.  
...
이미 프로그래매틱으로 거래되는 광고의 양은 증가하고 있습니다.  
몇몇의 에이전시들은 프로그래매틱 채널을 통해 최대한의 많은 미디어를 사고 싶어하고,  
몇몇의 브랜드들은 이미 프로그래매틱 바잉을 위해 인하우스팀을 꾸린 상태입니다.  

지금은 온라인 광고가 주로 프로그래매틱으로 거래되지만,  
미디어 기업들은 티비광고를 포함한 전통적인 미디어 또한 프로그래매틱으로 거래할 수 있는 방법을 찾고있습니다.  
...
```온라인 광고 노출을 실시간 경매를 통해 구매하고 판매하는 것```

여기서 실시간이란 웹페이지가 로딩하는 시간을 의미합니다.
이러한 경매는 주로 **ADexchange**나 **SSP**를 통해 이루어집니다.    

### - RTB는 어떻게 작동하나요?

```
한 개의 광고노출이 유저의 웹브라우저에서 로드
> 현재 페이지의 정보와 그 페이지를 보고 있는 유저의 정보가 애드익스체인지로 전달
> 애드익스체인지에서 그 노출에 대해서 가장 높은 금액을 낼 의향이 있는 광고주들을 대상으로 경매 진행  
> 비딩에 성공한 광고주의 광고가 그 유저의 웹페이지에 거의 즉각적으로 노출 
= 이 모든 과정은 순식간에 일어남. "실시간"
```

광고주는 주로 **DSP**를 기반으로 유저들의 이전 방문 사이트, 유저 행동 등 다양한 변수들을 고려하여 **어떤 광고 노출을 사야하는지, 얼마를 비딩에 투자해야하는지 다양한 변수들을 기반으로 얼마를 투자해야하는지** 결정하게 됩니다.  

Zappos가 어떤 유저가 그들의 사이트에서 특정 신발을 봤었다는 사실을 알 수 있다면,  
아마도 아마존이나 베스트바이보다 그 유저에게 광고를 노출시키기 위해서 더 많은 돈을 지불할 의향이 있을 것입니다.

즉, **실시간으로 광고주가 얼마나 지불할 의향이 있는지를 기반으로 한 광고 노출의 가격이 "실시간"으로 정해지는 것**이 RTB입니다.  

### - 이게 왜 중요하죠?  

```그룹이 아닌 개인에게 타게팅 가능``` 

역사적으로 광고주들은 웹사이트들을 광고를 노출시키기 위한 프록시(proxy?)로 활용해왔습니다.  
만약 광고주가 스포츠 팬들에게 노출하고 싶다면 스포츠 관련 사이트의 지면을 구매했지요.   
하지만 RTB가 등장하면서 광고주들은 **특정 사용자(개인)** 에게 그들의 광고를 타겟할 수 있게 되었습니다.    

### - RTB와 Programmatic은 같은 건가요?  

```아닙니다. RTB는 프로그래매틱의 한 종류일 뿐.```

어떤 종류의 프로그래매틱은 매체들이 그들의 인벤토리를 정해둔 가격으로 미리 판매를 할 수 있도록 합니다.  
실시간 경매인 RTB와는 다른 방식이죠.  
= **프로그래매틱 다이렉트(programmatic direct)** 혹은 **프로그래매틱 개런티드(programmatic guaranteed)** 라고 부릅니다.  

### - RTB는 왜 광고주들에게 좋을까요?  

```효율성```

**RTB의 등장으로 광고주들은 더이상 매체 혹은 애드네트워크와 직접적으로 협상할 필요가 없어졌습니다.**  
애드익스체인지나 다른 애드테크를 활용하여 다양한 사이트의 다양한 인벤토리에 접근할 수 있게 되었지요.    
그리고 그들에게 가장 중요하다고 판단되는 노출만을 골라서 비용을 지불할 수 있게 되었습니다.  체리픽커!    
적합하지 않은 유저에게 낭비되는 노출을 줄이고 비싼 인력을 줄일 수 있음. (슬픔)    

### - RTB는 매체한테 별로라든데 사실인가요?  

```광고주들이 인벤토리에 적은 비용을 지불하게되니 걱정이 될 수 있죠.```

하지만 익스체인지와 SSP에서 매체가 인벤토리의 최저판매금액 **(Price Floor)** 을 설정할 수 있습니다.  
...
```광고주와 매체가 지면을 사고 팔 수 있는 디지털 시장(digital marketplace)```

주로 실시간 경매로 거래가 이루어지며,  
주로 디스플레이, 비디오, 모바일 광고 인벤토리가 거래됩니다. 

### - 애드 익스체인지에서 누가 삽니까?  

```애드익스체인지가 허용하는 그 누구나 살 수 있습니다.```

주로 광고주와 에이전시는 **DSP**나 그들이 구축해놓은 비딩 기술을 활용하여 구매하고,  
**애드네트워크**나 다른 주체는 익스체인지로부터 구매합니다.  

### - 좋네요. 그럼 거래는 어떻게 이루어지나요?  

``` 애드 익스체인지는 광고 노출의 큰 풀(big pool of ad impressions)이라고 보면됩니다.```  

매체들은 광고 노출을 시장에 내놓고, 광고주는 **DSP**와 같은 기술을 활용하여 구매하고 싶은 노출만을 구매할 수 있습니다.   
이러한 결정은 광고가 노출되는 유저의 정보(이전에 방문했던 사이트, 현재시간, 디바이스 종류 등)을 기반으로 실시간으로 이루어집니다.    

### - 이게 왜 중요한가요?  

```광고를 구매하고 판매하기에 훨씬 효과적이고 효율적입니다.```

애드익스체인지는 광고주가 다양한 사이트의 다양한 지면을 한번에 구매할 수 있도록 합니다.
물론 인력협상 과정이 없이도요!

### - 그게 애드네트워크 아닌가요?  

```애드 네트워크는 주로 다양한 매체로부터 인벤토리를 모으고 인벤토리 묶음을 판매합니다. 이 자체가 애드네트워크의 수익모델이죠.```

하지만 **애드익스체인지**는 광고주들이 광고 노출이 정확하게 얼마에 거래되는지 볼 수 있기 때문에,  
**애드네트워크**보다 훨씬 투명합니다.(transparent)

사실은 많은 **애드네트워크**가 현재 인벤토리를 익스체인지에서 구매하고 있으며,  
몇몇은 익스체인지보다 **DSP**가 네트워크랑 더 비슷한 모습으로 변하고 있다고 말합니다.  

### - 그렇다면 Private 익스체인지는 뭔가요?  

```어떤 광고주가 인벤토리를 구매할 수 있는지, 얼마에 구매할 수 있는지에 대해 신중하게 선택하고 싶은 매체에 의해 사용됩니다.```

매체들은 열린 시장에 그들의 지면을 던져놓고 아무나 살 수 있도록 하는 것이 아니라, 
그들이 선호하는 광고주들이나 에이전시에게 그들의 지면을 오픈할 수 있습니다.    
이것은 자신들의 지면을 재판매할 수 있는 다른 네트워크나 익스체인지가 접근하지 못하게 막을 수도 있습니다.  

### - 그럼 SSP와 익스체인지의 차이는 뭔가요?  

```별로 없습니다.``` 

매체들은 다양한 익스체인지를 플러그인(plug in)하기 위해서 SSP를 사용합니다.  
하나의 SSP를 사용하는 것보다 좀더 많은 구매자들에게 인벤토리를 오픈한다면 경쟁이 붙어서 더 높은 금액에 팔 수 있기때문이죠.  
**SSP**는 익스체인지를 모으는 매체들에 의해서 사용되고, **익스체인지**는 노출을 모으는데 사용됩니다.  

### - 누가 이런 익스체인지를 운영하나요?  

```구글, 루비콘 프로젝트, 오픈X, 앱넥서스, 야후```

다양한 애드테크 기업들이 매체가 private 익스체인지를 만들 수 있는 기능을 제공하고 있습니다.  

### - 이 안에서는 누가 룰을 정하나요?  

```위에서 말한 익스체인지 오퍼레이터들이 어떠한 노출이 마켓안으로 유입되고, 광고주들에게 오픈되는지에 대한 책임은 가지고 있긴하죠.```

하지만 문제는 매일 수많은 매체들로부터 수많은 광고 노출이 익스체인지 안으로 들어옵니다.   
따라서 누가 사고 팔고있는지 하나하나 트랙킹하는 것은 거의 불가능하죠.  
즉, 구매자는 그들의 노출이 어디에 노출되는지,  
그리고 판매자는 누가 그들의 지면을 사는지 알 수 없습니다.  
...
구글은 뷰어블 노출이 논뷰어블보다 더 잘팔리기를 기대하고 있습니다.   
...
### 헤더비딩은 좋은 아이디어같네요. 하지만 또 뭐가 문제인가요?  
